...
def extract_text_with_tika(file_path):
    """Извлечение текста с помощью Tika"""
    try:
...
def process_folder_simple(folder_path, output_file=None):
    ...
    output_file: (опционально) путь к файлу для сохранения результатов
    
    Возвращает:
    list: список словарей с результатами обработки файлов
          [{'file': имя_файла, 'text': текст}, ...]
          или пустой список, если папка не существует
    """
        ...
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                for result in results:
                    if result['text']:  # Сохраняем только файлы с извлеченным текстом
    ...
    # Вывод превью в консоль
    if results:
        successful = [r for r in results if r.get('text')]
        print(f"\nОбработка завершена. Успешно: {len(successful)}/{len(results)} файлов")
        
        if not output_file:  # Показываем превью только если не сохраняли в файл
            for result in successful[:3]:  # Показываем первые 3 успешных файла
                print(f"\nФайл: {result['file']}")
                print("="*80)
